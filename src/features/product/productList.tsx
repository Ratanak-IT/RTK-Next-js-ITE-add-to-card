...
export default function ProductList() {
  ...
  const filteredProducts = data?.content?.filter((product: any) => {
    const matchCategory = filters.categoryUuid
      ? product.category?.uuid === filters.categoryUuid
      : true;
    const matchBrand = filters.brandUuid
      ? product.brand?.uuid === filters.brandUuid
      : true;
  ...
  return (
    <div className="px-6 py-10">
      <select
        value={filters.categoryUuid ?? ""}
        onChange={(e) => handleFilter("categoryUuid", e.target.value)}
      >
      ...
      <select
        value={filters.brandUuid ?? ""}
        onChange={(e) => handleFilter("brandUuid", e.target.value)}
